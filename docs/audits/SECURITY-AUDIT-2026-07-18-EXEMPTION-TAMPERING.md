...
Status 2026-07-22: C1 (`d2000e5`), C2+C3 (`fe2ba7d`), C4 (`678f662`+`3823cea`), C5 (`8f29432`), C6 (`5181001`), H1 (`1906fdd`), H2 (WORKSPACE-GUARD `b4bafee`), H3 (WORKSPACE-GUARD `d47e3b1`), H4 (WORKSPACE-GUARD `1963fac`), H5 (WORKSPACE-VM `07a2c91`), M1 (`ac149b4`), M2 (WORKSPACE-GUARD `ded033c`), M3 (`1b5da3d`+`aa3a6a3`), M4 (`e48e2cc`), M5 (`f62f5fc`), L1 (`4a96a9a`), L2 (`dea059c`), L4 (WORKSPACE-GUARD `5c3c586`) all FIXED. L3 ACCEPTED-RISK: the `>/dev/null` ban is inherently a commit-time static control; runtime suppression by ad-hoc shell commands cannot be enforced by the commit-gate layer and falls to the guard binary's command policy (which mediates all git execution) plus auditd rules. Remaining operational items: root-side `sudo setcap -r /usr/bin/true` followed by `make sync-gtfobins` baseline refresh, deploy-ci of these changes, install-hooks-recursive, and guard binary install (install-guard-host-exec).
