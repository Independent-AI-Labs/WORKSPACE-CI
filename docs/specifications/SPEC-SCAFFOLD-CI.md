...
> coverage under `tests/unit/scaffold/ (4 suites)`. The companion
...
| 5 | `CI/tests/unit/scaffold/test_scaffold_ci.sh` | Unit tests: path math, parsing, validation, auto-insertion ordering. | Maintained by hand |
...
The generator **never reorders unannounced** user-listed hooks. If a project
...
- If it exists AND its contents differ from the rendered template AND
  ...
  to inspect the differences (per Rule 13: no unannounced overwrite).
- If it exists AND its contents differ from the rendered template AND
...
### 14.1 `tests/unit/scaffold/test_scaffold_ci.sh`
