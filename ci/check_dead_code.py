...
from ci.models import DeadCodeEntry, LoadedConfig


def _find_config_dir() -> Path:
    """Find CI config dir from CI_CONFIG_DIR env or by walking up to a 'config/' dir."""
    ...
    msg = "Cannot find CI config directory"
    raise FileNotFoundError(msg)


_CI_CONFIG = _find_config_dir()
DEFAULT_CONFIG_PATH = str(_CI_CONFIG / "dead_code.yaml")
...
def _compile_regex_safe(pattern: str) -> re.Pattern[str] | None:
    try:
...
def load_config(path: str) -> LoadedConfig:
    ...
    for pat_str in raw.get("ignored_name_patterns", []):
        pat = _compile_regex_safe(pat_str)
        if pat is not None:
            compiled.append(pat)
...
def main() -> None:
    ...
    scan_paths: list[str] = raw_config.get("scan_paths", ["ci"])
