...
# The rewriter's execute path requires the real (unguarded) git binary:
# its PATH-prepend bypass is defeated by the shell guard's PATH
# normalization, and /usr/bin/git.original is 0700 root. As agent the
# backup update-ref is guard-blocked (proven 2026-08-23). Execute-path
# tests therefore run only where real git is reachable: root context or
# sandboxed podman tiers. Dry-run/detection/abort tests always run.
_RW_CAN_EXECUTE=1
if [[ "$(id -u)" != "0" && "${WORKSPACE_CI_TEST_REAL_GIT:-0}" != "1" ]]; then
    _RW_CAN_EXECUTE=0
fi
_rw_execute_test() {
    # shellcheck disable=SC2120
    if [[ "$_RW_CAN_EXECUTE" != "1" ]]; then
        echo "  SKIP (operator/podman only): $1"
        return 0
    fi
    _run_test "$1" "$2"
}
...
# Identity is provided per-invocation via GIT_AUTHOR_* / GIT_COMMITTER_*
# env vars: the git guard blocks `git config` writes, including in tests.
...
_rw_execute_test "rewrite: single offending commit" _test_single_offending
...
_rw_execute_test "rewrite: multiple offending commits" _test_multiple_offending
...
_rw_execute_test "rewrite: pattern in middle of body" _test_middle_body
...
_rw_execute_test "rewrite: multiple blocked lines in one commit" _test_multi_lines
...
_rw_execute_test "rewrite: mixed case caught" _test_mixed_case
...
_rw_execute_test "rewrite: separator variants caught" _test_separators
...
_rw_execute_test "rewrite: empty body after removal" _test_empty_body
...
_rw_execute_test "rewrite: base SHA limits range" _test_base_sha

# === Test: preserves commit metadata ===
_test_preserves_metadata() {
    _make_repo
    cd "$TEST_TMP/repo"
    GIT_AUTHOR_NAME="SpecificAuthor" \
    GIT_AUTHOR_EMAIL="specific@author.com" \
    GIT_COMMITTER_NAME="SpecificAuthor" \
    GIT_COMMITTER_EMAIL="specific@author.com" \
...
_rw_execute_test "rewrite: preserves author and date" _test_preserves_metadata
...
_rw_execute_test "rewrite: backup branch created" _test_backup_created
...
_rw_execute_test "rewrite: backup restore recovers original" _test_backup_restore
...
_rw_execute_test "rewrite: unicode preserved" _test_unicode
...
_rw_execute_test "rewrite: backup ref does not pollute verification" _test_backup_no_pollute
...
_rw_execute_test "rewrite: idempotent second run" _test_idempotent
...
_rw_execute_test "rewrite: line actually removed from history" _test_line_actually_removed
