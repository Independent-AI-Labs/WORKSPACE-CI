...
class TestLoadConfig:
    ...
    def test_entry_points_loaded(self, tmp_path: Path) -> None:
        ...
        loaded = load_config(str(cfg_file))
        config = cast(DeadCodeConfig, loaded.config)
        assert "ci/scripts/*.py" in config.entry_point_patterns
...
class TestIsIgnored:
    ...
    def test_exact_match(self) -> None:
        assert _is_ignored("ci/foo.py", ["ci/foo.py"]) is True

    def test_prefix_match(self) -> None:
        assert _is_ignored("ci/__pycache__/mod.pyc", ["ci/__pycache__"]) is True

    def test_no_match(self) -> None:
        assert _is_ignored("ci/core/utils.py", ["ci/__pycache__"]) is False
...
class TestPrintReport:
    ...
    def test_report_shows_file_and_line(
        ...
        items = [_make_item("func", "function", "ci/core.py", 42)]
        print_report(items)
        out = capsys.readouterr().out
        assert "ci/core.py:42" in out
