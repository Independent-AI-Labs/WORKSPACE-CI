...
from ci.dead_code_analyzer import (
    CrossReferenceGraph,
    DeadCodeConfig,
    Definition,
    ModuleInfo,
...
class TestHelpers:
    ...
    def test_is_entry_point_match(self) -> None:
        cfg = DeadCodeConfig(
            entry_point_patterns=["ci/scripts/ci/*.py"],
            ignored_names=set(),
            ignored_name_regexes=[],
            reference_only_patterns=[],
        )
        assert is_entry_point("ci/scripts/ci/check_dead_code.py", cfg) is True
    ...
    def test_is_entry_point_no_match(self) -> None:
        ...
        assert is_entry_point("ci/core/utils.py", cfg) is False
    ...
    def test_is_reference_only_false(self) -> None:
        ...
        assert is_reference_only("ci/core/utils.py", cfg) is False
...
class TestFindDeadCode:
    ...
    def test_dead_module(self) -> None:
        ...
        graph.add(
            self._make_info(
                "ci/orphan.py",
                "ci.orphan",
                definitions=[_make_def("func", file="ci/orphan.py")],
            )
        )
    ...
    def test_entry_point_not_dead(self) -> None:
        ...
        cfg = DeadCodeConfig(
            entry_point_patterns=["ci/scripts/*.py"],
            ignored_names=set(),
            ignored_name_regexes=[],
            reference_only_patterns=[],
        )
        ...
        graph.add(
            self._make_info(
                "ci/scripts/run.py",
                "ci.scripts.run",
                definitions=[_make_def("func", file="ci/scripts/run.py")],
            )
        )
    ...
    def test_dead_function(self) -> None:
        ...
        graph.add(
            self._make_info(
                "ci/a.py",
                "ci.a",
                definitions=[_make_def("unused_func", file="ci/a.py")],
            )
        )
        graph.add(self._make_info("ci/b.py", "ci.b", imports=["ci.a"]))
        dead = find_dead_code(graph, EMPTY_CONFIG)
    ...
    def test_live_function(self) -> None:
        ...
        graph.add(
            self._make_info(
                "ci/a.py",
                "ci.a",
                definitions=[_make_def("used_func", file="ci/a.py")],
                imports=["ci.b"],  # bidirectional import so b is not dead
            )
        )
        graph.add(
            self._make_info(
                ...
                "ci.b",
                imports=["ci.a"],
                references=[Reference(name="used_func", file="ci/b.py", line=1)],
            )
        )
        dead = find_dead_code(graph, EMPTY_CONFIG)
        assert len(dead) == 0
    ...
    def test_init_files_excluded(self) -> None:
        ...
        graph = CrossReferenceGraph()
        graph.add(self._make_info("ci/__init__.py", "ami"))
        dead = find_dead_code(graph, EMPTY_CONFIG)
        assert len(dead) == 0
    ...
    def test_dunder_methods_excluded(self) -> None:
        ...
        graph.add(
            self._make_info(
                ...
                definitions=[
                    _make_def(
                        "__init__", kind="method", file="ci/a.py", is_dunder=True
                    ),
                ],
                imports=["ci.b"],
            )
    ...
    def test_exported_names_excluded(self) -> None:
        ...
        graph.add(
            self._make_info(
                ...
                "ci.a",
                definitions=[
                    _make_def("public_api", file="ci/a.py", is_exported=True),
                ],
                imports=["ci.b"],
            )
        )
    ...
    def test_unimported_module_skips_individual_defs(self) -> None:
        ...
        graph.add(
            self._make_info(
                ...
                "ci.orphan",
                definitions=[
                    _make_def("func_a", file="ci/orphan.py"),
                    _make_def("func_b", file="ci/orphan.py"),
                ],
            )
        )
        dead = find_dead_code(graph, EMPTY_CONFIG)
    ...
    def test_methods_always_skipped(self) -> None:
        ...
        graph.add(
            self._make_info(
                ...
                "ci.a",
                definitions=[
                    _make_def("MyClass.do_work", kind="method", file="ci/a.py"),
                ],
                imports=["ci.b"],
            )
        )
    ...
    def test_private_names_skipped(self) -> None:
        ...
        graph.add(
            self._make_info(
                ...
                "ci.a",
                definitions=[
                    _make_def("_helper", kind="function", file="ci/a.py"),
                ],
                imports=["ci.b"],
            )
        )
    ...
    def test_public_name_used_internally_not_dead(self) -> None:
        ...
        graph.add(
            self._make_info(
                "ci/a.py",
                "ci.a",
                definitions=[_make_def("helper", kind="function", file="ci/a.py")],
                references=[Reference(name="helper", file="ci/a.py", line=20)],
                imports=["ci.b"],
            )
        )
        graph.add(self._make_info("ci/b.py", "ci.b", imports=["ci.a"]))
    ...
    def test_public_name_never_referenced_is_dead(self) -> None:
        ...
        graph.add(
            self._make_info(
                "ci/a.py",
                "ci.a",
                definitions=[_make_def("OrphanClass", kind="class", file="ci/a.py")],
            )
        )
        graph.add(self._make_info("ci/b.py", "ci.b", imports=["ci.a"]))
        dead = find_dead_code(graph, EMPTY_CONFIG)
    ...
    def test_module_only_imported_by_tests_is_dead(self) -> None:
        ...
        assert mod_dead[0].definition.name == "ci.orphan"
    ...
    def test_name_only_referenced_by_tests_is_dead(self) -> None:
        ...
        graph.add(
            self._make_info(
                "ci/a.py",
                "ci.a",
                definitions=[_make_def("DeadFunc", kind="function", file="ci/a.py")],
                imports=["ci.b"],
            )
        )
        graph.add(self._make_info("ci/b.py", "ci.b", imports=["ci.a"]))
    ...
    def test_name_referenced_by_prod_and_tests_is_alive(self) -> None:
        ...
        graph.add(
            self._make_info(
                "ci/a.py",
                "ci.a",
                definitions=[_make_def("LiveFunc", kind="function", file="ci/a.py")],
                imports=["ci.b"],
            )
        )
        graph.add(
            self._make_info(
                ...
                "ci.b",
                imports=["ci.a"],
                references=[Reference(name="LiveFunc", file="ci/b.py", line=3)],
            )
        )
        graph.add(
            self._make_info(
