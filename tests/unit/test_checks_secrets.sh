...
test_secrets_uses_one_process_and_gitignore_allowlist() {
    _source_lib
    printf 'ignored[one].txt\n' > .gitignore
    printf 'clean\n' > clean.txt
    printf 'ignored\n' > 'ignored[one].txt'
    git add .gitignore clean.txt 'ignored[one].txt'
...
MOCK_EOF
    ...
    ! grep -Fq 'clean.txt' "$GITLEAKS_CONFIG_CAPTURE"
}
_run_test "secrets: one process with escaped gitignore allowlist" test_secrets_uses_one_process_and_gitignore_allowlist
