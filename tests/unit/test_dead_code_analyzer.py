...
class TestPathToModuleName:
    ...
    def test_simple_module(self) -> None:
        assert path_to_module_name("ci/core/utils.py") == "ci.core.utils"

    def test_init_file(self) -> None:
        assert path_to_module_name("ci/core/__init__.py") == "ci.core"

    def test_top_level(self) -> None:
        assert path_to_module_name("setup.py") == "setup"

    def test_nested_path(self) -> None:
        result = path_to_module_name("ci/check_dead_code.py")
        assert result == "ci.check_dead_code"
...
class TestReferenceCollector:
    ...
    def test_submodule_import_tracking(self) -> None:
        c = self._collect("from ci.core import utils")
        assert "ci.core" in c.imports
        assert "ci.core.utils" in c.imports
...
class TestCrossReferenceGraph:
    ...
    def test_is_module_imported_exact(self) -> None:
        graph = CrossReferenceGraph()
        graph.add(self._make_info("b.py", "b", imports=["ci.core.utils"]))
        assert graph.is_module_imported("ci.core.utils") is True
        assert graph.is_module_imported("ci.other") is False
        graph.add(self._make_info("b.py", "b", imports=["ci.core.utils.helpers"]))
        assert graph.is_module_imported("ci.core.utils") is True
        assert graph.is_module_imported("ci.core.utils.helpers") is True
        graph.add(self._make_info("b.py", "b", imports=["ci.other"]))
        assert graph.is_module_imported("ci.core.utils") is True
