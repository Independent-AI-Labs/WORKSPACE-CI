...
def test_hook_applies_skips_non_mandatory() -> None:
    ...
    assert _hook_applies(hook, {"rust"}) is False


def test_hook_applies_includes_mandatory_hook() -> None:
    hook = HookEntry(
        id="x",
        kind="shell",
        entry="x",
    ...
    assert _hook_applies(hook, {"rust"}) is True


def test_hook_applies_filters_languages() -> None:
    hook = HookEntry(
        id="x",
        kind="shell",
        entry="x",
    ...
    assert _hook_applies(hook, {"rust"}) is False
    assert _hook_applies(hook, {"python"}) is True
...
def test_check_hooks_rendered_finds_present_hooks(tmp_path: Path) -> None:
    ...
    assert _check_hooks_rendered(repo, manifest) == []
...
def test_check_hooks_rendered_flags_missing(tmp_path: Path) -> None:
    ...
    issues = _check_hooks_rendered(repo, manifest)
    assert len(issues) == 1
    assert "ci-lint" in issues[0]


def test_check_hooks_rendered_handles_missing_hooks_dir(tmp_path: Path) -> None:
    issues = _check_hooks_rendered(tmp_path, HooksManifest())
    assert any("hooks not found" in i for i in issues)


def test_check_hooks_rendered_accepts_exact_predecessor_before_deployment(
    tmp_path: Path,
) -> None:
    hooks = tmp_path / ".git" / "hooks"
    hooks.mkdir(parents=True)
    body = (
        "# AUTO-GENERATED by ../CI/scripts/generate-hooks\n"
        'source "${_ROOT}/../CI/lib/ci.sh"\n'
    )
    for stage in ("pre-commit", "commit-msg", "pre-push"):
        (hooks / stage).write_text(body)
    assert _check_hooks_rendered(tmp_path, HooksManifest()) == []
