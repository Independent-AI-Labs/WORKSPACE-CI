...
from ci._docker_versions import (
    DockerHubTag,
    _check_image_tag,
...
class TestResolveDockerHubApiPath:
    ...
    def test_user_image(self) -> None:
        assert _resolve_docker_hub_api_path("bitnci/redis") == "bitnci/redis"
...
class TestPickBestTag:
    ...
    def test_picks_highest_semver(self) -> None:
        assert _pick_best_tag(_tags("1.0.0", "2.0.0", "1.5.0")) == "2.0.0"

    def test_skips_banned_tags(self) -> None:
        assert _pick_best_tag(_tags("latest", "1.0.0")) == "1.0.0"

    def test_skips_prerelease(self) -> None:
        assert _pick_best_tag(_tags("2.0.0-rc1", "1.0.0")) == "1.0.0"

    def test_skips_variant_suffixes(self) -> None:
        assert _pick_best_tag(_tags("2.0.0-alpine", "1.0.0")) == "1.0.0"

    def test_returns_none_for_empty(self) -> None:
        assert _pick_best_tag([]) is None

    def test_returns_none_for_only_banned(self) -> None:
        assert _pick_best_tag(_tags("latest", "stable")) is None

    def test_skips_empty_name(self) -> None:
        assert _pick_best_tag(_tags("", "1.0.0")) == "1.0.0"

    def test_non_semver_candidate(self) -> None:
        assert _pick_best_tag(_tags("1.0.0-build.42")) == "1.0.0-build.42"
...
class TestGetDockerHubLatest:
    ...
    @patch("ci._docker_versions.urllib.request.urlopen")
...
class TestCheckImageTag:
    ...
    @patch("ci._docker_versions._get_docker_hub_latest")
...
class TestCheckCompose:
    ...
    @patch("ci._docker_versions._check_image_tag")
