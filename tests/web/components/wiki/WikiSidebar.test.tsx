...
const mockBranding: Branding = {
  ...
  grafana_dashboards: [
    {
      title: "LEADERBOARD",
      url: "http://localhost:3030/d/gateway-cost-leaderboard/leaderboard",
    },
    {
      title: "USAGE & COSTS",
      url: "http://localhost:3030/d/gateway-cost-usage/usage",
    },
    {
      title: "OPERATIONS",
      url: "http://localhost:3030/d/gateway-ops-health/ops",
    },
  ],
...
describe("WikiSidebar", () => {
  it("renders navigation links", () => {
    ...
    expect(screen.getByText("Hook Configs")).toBeInTheDocument();
  ...
  it("renders all nav items", () => {
    renderSidebar();
    expect(screen.getByText("System Policies")).toBeInTheDocument();
    expect(screen.getByText("Sandbox Configs")).toBeInTheDocument();
  ...
  it("renders [0] for Runtime Hooks", () => {
    renderSidebar();
    const runtimeHooksLink = screen.getByText("System Policies").closest("a");
  ...
  it("places AI Governance before LLM Gateway", () => {
    ...
    const labels = Array.from(links).map((l) =>
      l.textContent?.replace(/\[\d+\]/g, "").trim(),
    );
  ...
  it("places AI Governance after Sandbox Configs", () => {
    const { container } = renderSidebar();
    const links = container.querySelectorAll(".wiki-sidebar__link");
    const labels = Array.from(links).map((l) =>
      l.textContent?.replace(/\[\d+\]/g, "").trim(),
    ...
    const standardsIdx = labels.indexOf("AI Governance");
    const guardIdx = labels.indexOf("Sandbox Configs");
  ...
  it("renders dividers around LLM Gateway, Static Analysis, and Playground group", () => {
    ...
    const items = Array.from(
      container.querySelectorAll(".wiki-sidebar__nav > li"),
    );
    ...
    const integrationIdx = labels.findIndex((l) =>
      l.includes("Integration Guide"),
    );
    expect(items[llmIdx]).toHaveClass("wiki-sidebar__divider");
    expect(items[integrationIdx]).toHaveClass("wiki-sidebar__divider");
    const checksLi =
      items[labels.findIndex((l) => l.includes("Static Analysis"))];
    const playgroundLi =
      items[labels.findIndex((l) => l.includes("Playground"))];
