...
export interface ReadmeLinkContext {
  ...
  branch?: string
  projectSlug?: string
  /** Inject a consumer-specific asset resolver. If omitted, no local asset rewriting occurs. */
  resolveAssetUrl?: (projectSlug: string, path: string) => string
}
...
export function rewriteRelativeImageSrc(
  ...
  if (ctx.resolveAssetUrl && ctx.projectSlug) {
    return ctx.resolveAssetUrl(ctx.projectSlug, path)
  }
