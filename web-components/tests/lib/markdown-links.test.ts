...
describe('rewriteRelativeImageSrc', () => {
  const ctx = { repoUrl: 'https://github.com/org/repo', branch: 'main' }
  const resolveAssetUrl = (projectSlug: string, path: string) =>
    `/api/project-asset?project=${encodeURIComponent(projectSlug)}&path=${encodeURIComponent(path)}`
  const localCtx = { ...ctx, projectSlug: 'workspace-gateway', resolveAssetUrl }

  it('uses the injected resolver when projectSlug and resolveAssetUrl are provided', () => {
  ...
  it('strips a leading ./ prefix before invoking the resolver', () => {
  ...
  it('ignores projectSlug and falls back to raw github url when no resolver is provided', () => {
    expect(rewriteRelativeImageSrc('res/gateway-screenshot.jpg', { ...ctx, projectSlug: 'workspace-gateway' })).toBe(
...
describe('buildReadmeMarked', () => {
  const resolveAssetUrl = (projectSlug: string, path: string) =>
    `/api/project-asset?project=${encodeURIComponent(projectSlug)}&path=${encodeURIComponent(path)}`
  const ctx = { repoUrl: 'https://github.com/org/repo', branch: 'main', projectSlug: 'workspace-gateway', resolveAssetUrl }
