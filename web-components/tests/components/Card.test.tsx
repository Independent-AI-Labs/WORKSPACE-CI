...
import type { CardItem } from '../../src/types/card'
...
describe('Card', () => {
  it('renders title as a link when href is provided', () => {
    const { container } = render(<Card item={makeItem({ href: '/test' })} />)
  ...
  it('renders as an article when no href', () => {
    const { container } = render(<Card item={makeItem()} />)
  ...
  it('renders title in code when monoTitle is true', () => {
    const { container } = render(
      <Card item={makeItem({ monoTitle: true, title: 'my_func' })} />,
    )
  ...
  it('renders title in span when monoTitle is false', () => {
    const { container } = render(
      <Card item={makeItem({ monoTitle: false, title: 'My Card' })} />,
    )
  ...
  it('renders subtitle when provided', () => {
    render(<Card item={makeItem({ subtitle: 'A subtitle' })} />)
  ...
  it('does not render footer CTA link for href-only cards', () => {
    const { container } = render(<Card item={makeItem({ href: '/detail' })} />)
  ...
  it('renders external GitHub link when repoUrl is set', () => {
    const { container } = render(
      <Card item={makeItem({ href: '/detail', repoUrl: 'https://github.com/Org/Repo' })} />,
    )
  ...
  it('renders icon when provided', () => {
    const { container } = render(
      <Card item={makeItem({ icon: 'ri-settings-3-line' })} />,
    )
  ...
  it('renders theme logo when logoPath is provided instead of icon', () => {
    const { container } = render(
      <Card item={makeItem({ icon: 'ri-settings-3-line', logoPath: '/LOGO.png' })} />,
    )
