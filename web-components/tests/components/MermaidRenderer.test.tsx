...
import type { MermaidController, MermaidRunner } from '../../src/lib/mermaid-diagram'
import { resetMermaidRunQueue } from '../../src/lib/mermaid-run-queue'
...
const { mountMermaidDiagram, fakeControllers } = vi.hoisted(() => {
  ...
  const mount = vi.fn((_frame: HTMLElement): MermaidController => {
    void _frame
...
vi.mock('../../src/lib/mermaid-diagram', async () => {
  const actual = await vi.importActual<typeof import('../../src/lib/mermaid-diagram')>(
    '../../src/lib/mermaid-diagram',
  )
  return {
    ...actual,
    mountMermaidDiagram,
